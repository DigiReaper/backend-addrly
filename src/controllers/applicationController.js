...
async function addToWaitlist(
	...
	try {
			...
			.upsert(
				{
					...
					source_reference: applicant_id,
					status: "active",
					metadata: {
						added_via: "application_submission",
		...
		if (error && error.code == "PGRST116") {
			// ignore duplicate email conflicts
...
class ApplicationController {
	...
	async submit(req, res) {
		try {
			...
			const { applicant_email, applicant_name, answers, submitted_links } =
				req.body;
			...
			if (!doc.is_active) {
				return res.status(400).json({
					error: "This date-me-doc is no longer accepting applications",
				});
			}
			...
			await addToWaitlist(applicant_name, applicant_email, application.id);
		...
		try {
          ...
          `
				...
				if (application.applicant_user_id && doc.user_profiles) {
					const ownerUrls = [];
					const applicantUrls = Object.values(
						application.submitted_links || {}
					);
					...
					await supabaseAdmin.from("matchmaking_scores").insert({
						application_id: applicationId,
				...
				if (ownerProfile) {
					...
					const compatibilityAnalysis =
						await psychologicalAnalyzer.calculateCompatibility(
							ownerProfile,
					...
					if (compatibilityAnalysis.success) {
						...
						const applicationMatch =
							await psychologicalAnalyzer.analyzeApplicationMatch(
								doc.preferences,
								ownerProfile,
								application.answers,
								profileAnalysis.profile
						...
						// Combine scores
						const overallScore =
							compatibilityAnalysis.compatibility.overall_compatibility_score *
								0.7 +
							(applicationMatch.success
								? applicationMatch.analysis.preference_match_score * 0.3
								: 0);
						...
						if (!existingScore) {
							await supabaseAdmin.from("matchmaking_scores").insert({
								...
								compatibility_breakdown:
									compatibilityAnalysis.compatibility.compatibility_breakdown,
								recommendation:
									compatibilityAnalysis.compatibility.recommendation,
							});
	...
	async getStatus(req, res) {
        ...
        `
			...
			if (!req.user && email) {
				...
				return res
					.status(400)
					.json({ error: "Email required for unauthenticated access" });
			}
	...
	async analyzeMatch(req, res) {
		try {
			...
			if (!docOwnerProfile || !applicantProfile) {
				return res
					.status(400)
					.json({ error: "Both owner and applicant profiles are required" });
			}
			...
			const answerAnalysis = {
				...
				recommendation:
					compatibilityData.overall_compatibility_score > 70
						? "strong_match"
						: "potential_match",
			};
        ...
        `
			...
			const scoredApplications = applications.map((app) => {
				...
				if (userProfile.interests && applicant.interests) {
					...
					const commonInterests = applicantInterests.filter((i) =>
						userInterests.has(i)
					);
					const interestScore =
						(commonInterests.length /
							Math.max(userInterests.size, applicantInterests.length)) *
						40;
					score += interestScore;
					factors.push({
						name: "Interests",
				...
				if (userProfile.core_values && applicant.core_values) {
					...
					const valueScore =
						(commonValues.length /
							Math.max(userValues.size, applicantValues.length)) *
						30;
					score += valueScore;
					factors.push({
						name: "Values",
				...
				if (userProfile.location && applicant.location) {
					const sameLocation =
						...
						applicant.location.toLowerCase();
				...
				if (
					...
					applicant.lifestyle_preferences
				) {
					...
					if (
						userLifestyle.exercise_frequency ===
						applicantLifestyle.exercise_frequency
					)
						lifestyleScore += 5;
					if (userLifestyle.drinking === applicantLifestyle.drinking)
						lifestyleScore += 5;
					if (userLifestyle.smoking === applicantLifestyle.smoking)
						lifestyleScore += 5;
